...
<ParamField body="identifier" type="string" default="none">
  ...
  If this matches a unit in the portfolios you have specified (or in all of your portfolios if you've specified none) that unit will be updated, otherwise a new one will be created.
</ParamField>
...
Returned if an external reference is supplied, without supplying any portfolio IDs
```json No portfolio IDs supplied for unit with external reference
{
   "error": {
    "type": "no_portfolio_ids",
    "message": "You must provide at least one portfolio ID if you are providing an external reference."
  }
}
...
Returned if attempting to add an existing external reference to another existing unit
```json Can not add existing external reference to other unit
{
   "error": {
    "type": "existing_external_ref",
    "message": "You have attempted to add an existing external reference to another existing unit that does not have the provided external reference.."
  }
}
