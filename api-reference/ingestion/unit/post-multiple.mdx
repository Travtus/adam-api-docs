---
title: "Create/Update units"
api: "POST https://api.travtus.com/units/multiple/"
description: "This endpoint creates or updates multiple units."
---

The units need to be created or updated against a portfolio.

If a portfolio id is not provided for the mulit-create/update endpoint, the operation will fail.
...
<ParamField body="portfolio" type="string" deafault="none" required>
  The ID of the portfolio you want to create or update the units against.
</ParamField>
...
<ParamField body="units" type="[object]" default="none" required>
  ...
  <Expandable title="unit">
    ...
    <ParamField body="adress" type="object" default="none">
...
Returned if no portfolio ID is passed in the body.
```json No portfolio ID supplied
{
   "error": {
    "type": "no_portfolio_id",
    "message": "You must provide a portfolio ID in the request body."
  }
}
...
Returned if no portfolio for the provided portfolio ID is found
```json Portfolio with provided ID not found
{
   "error": {
    "type": "portfolio_not_found",
    "message": "We could not find a portfolio with the ID you have provided."
  }
}
