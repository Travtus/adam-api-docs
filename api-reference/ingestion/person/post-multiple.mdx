...
The API will attempt to match these records with existing person records within the scope of the provided portfolios.

If a match is found, the existing person record will be updated with the information provided.

If no match is found, a new person record will be created and scoped to the provided portfolios.

If no portfolios are provided, any updated or created person records will be scoped to all portfolios.
...
<ParamField body="persons" type="[object]" default="none" required>
  ...
  <Expandable title="properties">
    ...
    <ParamField header="portfolios" type="[string]" default="none">
      A list of portfolio IDs to be used when creating or updating the person record.

      The portfolios will be used for matching existing person records and for scoping newly created ones.
    </ParamField>
  </Expandable>
</ParamField>
