...
api: "POST https://api.travtus.com/ingestion/persons/"
...
It attempts to match the person information provided with existing records, within the scope of the portfolio IDs provided in the `Portfolio-IDs` header. If no match is found, a new person record will be created and automatically scoped to the portfolios with the provided IDs. If a match is found, the existing person record will be updated with the information provided.
...
If you do not provide portfolio IDs, the newly created or updated record will not be scoped to any portfolio and will instead be visible company-wide.
...
<ParamField body="external_ref" type="string" default="none">
  An external reference for the person you want to create. This is typically an identifier you may use for person records in your own system. Please not that this needs to be unique across all of your portfolios. If you provide an external_ref that is already present in your company, the person record with the provided external_ref will be updated instead, regardless of the portfolio scoping options you have provided.
</ParamField>
...
--header 'Content-Type: application/json' \
--header 'Authorization: bearer <token>' \
--header 'Portfolio-Ids: ["portfolio-id-1", "portfolio-id-2"]' \  
--data-raw '{
    "email_address": "test.user@travtus.com",
    "first_name": "Test",
    "last_name": "User",
