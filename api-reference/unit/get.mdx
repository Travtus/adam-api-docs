---
title: "Get Unit"
api: "GET https://api.travtus.com/units/{externalref}?group-id={group-id}/"
description: "This endpoint retrieves infromation about one unit with the provided external reference for the provided group."
---

You must provide a group id as external references are only unique within a group.
...
<ParamField path="group-id" type="string" default="none" required>
  The group for the unit to be retrieved.
</ParamField>
...
```json No group id provided
{
   "error": {
    "type": "missing_group_id",
    "message": "Your request does not include a group-id query parameter. You must provide one as external references are only unique within a group."
  }
}
```
