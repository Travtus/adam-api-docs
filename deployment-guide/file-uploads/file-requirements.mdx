...
## File Requirements & Validation
  ...
  - **Supported formats:** csv (UTF-8), json(UTF-8), json.gz. Gzipped CSV (`.csv.gz`) is not supported.
  - **S3 key shape:** `companyName_companyId/TYPE-SUBTYPE/filename.ext`.
  - **CSV specifics:** Header row required. Delimiter must match the mapping config (`delimiter`); no auto-detect. Fields should not span multiple lines.
  ...
  must match configured headers [Data structure](./data-types-and-data-structure); extra/missing columns fail mapping. Portfolio ID must be present or derivable for non-default types.
...
## Pre-flight Checklist for Uploaders
  ...
  - [ ] Delimiter matches mapping config or default to ','
